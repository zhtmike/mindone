...
from .. import functional_compat as F
...
class LlamaMLP(nn.Cell):
    ...
    def construct(self, x):
        if self.config.pretraining_tp > 1:
            ...
            gate_proj = ops.cat([F.linear(x, gate_proj_slices[i]) for i in range(self.config.pretraining_tp)], axis=-1)
            up_proj = ops.cat([F.linear(x, up_proj_slices[i]) for i in range(self.config.pretraining_tp)], axis=-1)

            intermediate_states = (self.act_fn(gate_proj) * up_proj).split(slice, axis=2)
            down_proj = [
                F.linear(intermediate_states[i], down_proj_slices[i]) for i in range(self.config.pretraining_tp)
            ]
            down_proj = ops.sum(ops.stack(down_proj), dim=0)
        else:
...
class LlamaAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs,
        ...
        attn_weights = ops.matmul(query_states, key_states.swapaxes(2, 3)) / ms.numpy.sqrt(self.head_dim)
        ...
        if self.config.pretraining_tp > 1:
            ...
            attn_output = ops.sum(
                ops.stack([F.linear(attn_output[i], o_proj_slices[i]) for i in range(self.config.pretraining_tp)]),
                dim=0,
            )
        else:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        dtype = input_tensor.dtype, input_tensor
