...
def apply_rotary_emb_flashatt(x, cos, sin, interleaved=False):
    """
...
class Qwen2_5_VLPatchMerger(nn.Cell):
    def __init__(self, dim: int, context_dim: int, spatial_merge_size: int = 2) -> None:
        ...
        self.mlp = nn.SequentialCell(
            mint.nn.Linear(self.hidden_size, self.hidden_size), mint.nn.GELU(), mint.nn.Linear(self.hidden_size, dim)
        )
...
class Qwen2_5_VLVisionFlashAttention2(nn.Cell):
...
class Qwen2_5_VLVisionAttention(nn.Cell):
    ...
    def construct(
        ...
        k = k.transpose(0, 1)
        v = v.transpose(0, 1)
        attn_weights = mint.matmul(q, k.transpose(1, 2)) / math.sqrt(self.head_dim)
        attn_weights = attn_weights + attention_mask
        attn_weights = F.softmax(attn_weights, dim=-1, dtype=ms.float32).to(q.dtype)
...
class Qwen2_5_VLVisionSdpaAttention(nn.Cell):
    def __init__(self, dim: int, num_heads: int = 16) -> None:
        super().__init__()
        self.num_heads = num_heads
        self.qkv = mint.nn.Linear(dim, dim * 3, bias=True)
    ...
    def construct(
        ...
        attention_mask = mint.ones([seq_length, seq_length], dtype=ms.bool_)
        for i in range(1, len(cu_seqlens)):
            attention_mask[cu_seqlens[i - 1] : cu_seqlens[i], cu_seqlens[i - 1] : cu_seqlens[i]] = False

        attn_output = ops.speed_fusion_attention(
            q, k, v, self.num_heads, "SBH", atten_mask=attention_mask, scale=1 / math.sqrt(q.shape[-1]), keep_prob=1.0
        )[0]
        attn_output = attn_output.reshape(seq_length, -1)
        attn_output = self.proj(attn_output)
        return attn_output
...
class Qwen2_5_VLVisionBlock(nn.Cell):
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def construct(self, hidden_states: Tensor, grid_thw: Tensor) -> Tensor:
        """
        Args:
        ...
        cu_window_seqlens = ms.tensor(cu_window_seqlens, dtype=ms.int32)
...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    as the weights of the module stays untouched. The only required change would be on the construct pass
    ...
    def construct(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "Qwen2_5_VLModel is using Qwen2_5_VLFlashAttention2, but `ops.flash_attention_score` does not support "
...
class Qwen2_5_VLSdpaAttention(Qwen2_5_VLAttention):
    """
    Qwen2 attention module using ops.speed_fusion_attention. This module inherits from
    ...
    def construct(
        ...
        if output_attentions:
            # TODO: Improve this warning with e.g. `model.config.attn_implementation = "manual"` once this is implemented.
            logger.warning_once(
                "Qwen2_5_VLModel is using Qwen2_5_VLSdpaAttention, but `ops.speed_fusion_attention` does not support "
                "`output_attentions=True`. Falling back to the manual attention implementation, "
                "but specifying the manual implementation will be required from Transformers version v5.0.0 onwards. "
                'This warning can be removed using the argument `attn_implementation="eager"` when loading the model.'
            )
        ...
        attn_output = ops.speed_fusion_attention(
            ...
            keep_prob=1 - self.attention_dropout if self.training else 1.0,
        )[0]
    ...
    def construct(
        ...
        **kwargs,
        ...
        Args:
            ...
            cache_position (`Tensor` of shape `(sequence_length)`, *optional*):
                Indices depicting the position of the input sequence tokens in the sequence.
            position_embeddings (`Tuple[Tensor, Tensor]`, *optional*):
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention / SPDA version of Qwen2_5_VL. Make sure to "
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        Args:
            ...
            target_length (`int`):
                The target length: when generating with static cache, the mask should be as long as the static cache, to account for the 0 padding, the part of
                    the cache that is not filled yet.
            dtype (`ms.Type`):
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def get_rope_index(
        ...
        Args:
            input_ids (`Tensor` of shape `(batch_size, sequence_length)`):
                ...
                it.
            image_grid_thw (`Tensor` of shape `(num_images, 3)`, *optional*):
                The temporal, height and width of feature shape of each image in LLM.
            video_grid_thw (`Tensor` of shape `(num_videos, 3)`, *optional*):
                The temporal, height and width of feature shape of each video in LLM.
            second_per_grid_ts (`Tensor` of shape `(num_videos)`, *optional*):
    ...
    def _get_image_nums_and_video_nums(
        ...
        Returns:
            image_nums (`Tensor` of shape `(batch_size, num_images_sample)`)
            video_nums (`Tensor` of shape `(batch_size, num_videos_sample)`)
        """
        image_token_id = self.config.image_token_id
        video_token_id = self.config.video_token_id
        vision_start_token_id = self.config.vision_start_token_id
