...
def _extract_into_tensor(arr: np.ndarray, timesteps: np.ndarray, broadcast_shape: Tuple[int, ...]):
    """
...
class GaussianDiffusionNP:
    ...
    def q_posterior_mean_variance(self, x_start: np.ndarray, x_t: np.ndarray, t: np.ndarray):
        """
    ...
    def p_mean_variance(
        self, model, x: np.ndarray, t: np.ndarray, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_variance, model_log_variance = {
                ...
                ModelVarType.FIXED_LARGE: (
                    np.concatenate([self.posterior_variance[1][None, ...], self.betas[1:]]),
                    np.log(np.concatenate([self.posterior_variance[1][None, ...], self.betas[1:]])),
                ),
        ...
        def process_xstart(x: np.ndarray):
    ...
    def _predict_xstart_from_eps(self, x_t: np.ndarray, t: np.ndarray, eps: np.ndarray):
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        new_mean = p_mean_var["mean"].astype(np.float32) + p_mean_var["variance"] * gradient.astype(np.float32)
        return new_mean
