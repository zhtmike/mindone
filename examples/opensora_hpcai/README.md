...
The saved checkpoint slices are stored at `./outputs` directory. To merge them into a single checkpoint, you may run

```bash
python tools/ckpt_combine_sp.py
```

The merged checkpoint is saved at `outputs/ckpt_full/rank_0/full_0.ckpt`.
...
To run the inference with very long videos, you also need to turn on the sequence parallel. For example, to inference a video with 512 frames and 512x512 resolution, you can run
...
The generated vdieo is saved at `./outputs` directory.
