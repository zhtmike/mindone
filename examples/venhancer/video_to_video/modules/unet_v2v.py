...
def sinusoidal_embedding(timesteps: Tensor, dim: int) -> Tensor:
    ...
    sinusoid = ops.outer(timesteps, ops.pow(10000, -ops.arange(half, dtype=ms.float32).div(half)))
...
class CrossAttention(nn.Cell):
    ...
    def construct(self, x: Tensor, context: Optional[Tensor] = None, mask: Optional[Tensor] = None):
        ...
        if q.shape[0] > self.max_bs:
            q_list = ops.chunk(q, q.shape[0] // self.max_bs, axis=0)
            k_list = ops.chunk(k, k.shape[0] // self.max_bs, axis=0)
            v_list = ops.chunk(v, v.shape[0] // self.max_bs, axis=0)
            out_list = []
            for q_1, k_1, v_1 in zip(q_list, k_list, v_list):
                out = self.attention(q_1, k_1, v_1)
                out_list.append(out)
            out = ops.cat(out_list, axis=0)
        else:
...
class TemporalConvBlock_v2(nn.Module):
    def __init__(
        ...
        ops.assign(self.conv4[-1].weight, ops.zeros_like(self.conv4[-1].weight))
        ops.assign(self.conv4[-1].bias, ops.zeros_like(self.conv4[-1].bias))
