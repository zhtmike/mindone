...
def _read_video_torchvision(ele: dict) -> Tensor:
    ...
    idx = mint.linspace(0, total_frames - 1, nframes).round().long()
    video = video[idx]
    return video
...
def _read_video_decord(
    ...
    nframes = smart_nframes(ele, total_frames=total_frames, video_fps=video_fps)
    idx = mint.linspace(0, total_frames - 1, nframes).round().long().tolist()
    video = vr.get_batch(idx).asnumpy()
    video = Tensor(video).permute(0, 3, 1, 2)  # Convert to TCHW format
    return video
